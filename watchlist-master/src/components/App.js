...
const App = React.createClass({
	...
	render() {
		...
		var headerStyles = {
			...
			width: "100%",
			background:
				"-webkit-linear-gradient(bottom, rgba(0,0,0,1) 0%,rgba(0,0,0,1) 50%,rgba(0,0,0,0) 100%)",
			color: "#fff",
			...
			<section style={wrapperStyle}>
				<header style={headerStyles}>
					...
					<div style={headerBodyStyles}>
						<p style={headerDescStyles}>
							A movie list app built by Sonal Sharma.
						</p>
						{this.props.authorized && (
							<button
								style={headerLogoutBtnStyles}
	...
	onSearchResultsItemClick(movie) {
		this.props.flux
			.getActions("watchlist")
			.watchlistAdd(movie, this.props.user);
	},

	onWatchListItemDelete(movie) {
		this.props.flux
			.getActions("watchlist")
			.watchlistRemove(movie, this.props.user);
	},
